...
直線ではない5個以上のグループを消すと、消えた場所に強化ブロックが1つ残ります。
強化ブロックはグループの大きさや位置に応じて変わります。

- ボム: 周囲3x3を消します。近くに別のボムがある場合は連鎖爆発します。
- 横ライン: 横1行を消します。
- 縦ライン: 縦1列を消します。
- クロス: 横1行と縦1列をまとめて消します。
- 大ボム: 周囲5x5を消します。
- 斜め: 斜め2方向を消します。
- 色消し: 隣接している通常ブロックと同じ色を盤面から消します。
- プラス: 上下左右2マス分を十字に消します。
- 市松: 市松模様の位置にあるブロックを消します。
- リング: 周囲5x5の外周だけを消します。

## ステージ制

ステージごとに目標スコア、手数、盤面の形が変わります。
ステージは手数を使い切ったタイミングで終了します。その時点で目標スコアとミッションの両方を達成しているとクリアです。
最後のステージをクリアすると `All Clear` になります。

失敗した場合は、結果画面に `スコア未達` や `ミッション未達` など、何が足りなかったかが表示されます。
...
3つ星の追加条件は `starScore`、`starScoreMultiplier` で調整できます。
...
`states` では、ゲーム状態ごとにどの表情を使うかを割り当てます。縦長で頭身のある立ち絵も使えるように、画面左側に高さ基準で大きく `object-fit: contain` で表示します。
...
ドラマ中の立ち絵は頭身の高い全身イラストを想定しているため、キャラの足元に名前ラベルは表示しません。発言者名はメッセージボックス上部に表示されます。
